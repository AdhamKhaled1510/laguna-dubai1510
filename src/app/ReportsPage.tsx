...
import { getOrders, clearAllOrders, clearOrdersByDate, deleteDailyReport, saveDailyReport, getDailyReports, getInvoices, clearAllData, updateInvoiceStatus, DailyReport, Order, Invoice } from './lib/orders';
import { ArrowLeft, BarChart3, Coffee, DollarSign, ShoppingBag, TrendingUp, Trash2, Calendar, ChevronDown, LayoutDashboard, Search, Receipt, Undo2, CheckCircle, XCircle, Clock, Printer } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from 'recharts';
import logoUrl from '@/assets/logo.png';

type ViewMode = 'today' | 'day' | 'month' | 'dashboard' | 'invoices';
...
export default function ReportsPage() {
  ...
  const [invFilter, setInvFilter] = useState<'all' | 'paid' | 'returned' | 'partial_return'>('all');

  const invFormatTime = (ts: number) => {
    const d = new Date(ts);
  ...
  const invCalcRefund = (inv: Invoice): number => {
  ...
  const invHandleReturn = async (inv: Invoice) => {
  ...
  const invHandlePartialReturn = async (inv: Invoice) => {
    ...
    const returnedItems = inv.items
      .filter(i => inputNames.some((n: string) => n === i.nameAr))
  ...
  const invHandlePrint = (inv: Invoice) => {
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 md:py-10">
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2">
          ...
          <button
            ...
            className={`shrink-0 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
              viewMode === 'invoices' ? 'bg-amber-100 text-amber-800' : 'bg-white text-stone-500 hover:bg-stone-50 border border-stone-200'
            }`}
          >
            <Receipt className="h-3.5 w-3.5 inline ml-1" />
            الفواتير
          </button>
        </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-stone-100">
                ...
                <p className="text-2xl font-bold text-emerald-600">
                  {invoices.reduce((s, inv) => s + inv.totalPrice - invCalcRefund(inv), 0).toLocaleString('ar-EG')} ج.م
                </p>
              </div>
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-stone-100">
                <p className="text-xs text-stone-400 mb-1">إجمالي المرتجعات</p>
                <p className="text-2xl font-bold text-red-500">
                  {invoices.reduce((s, inv) => s + invCalcRefund(inv), 0).toLocaleString('ar-EG')} ج.م
                </p>
              </div>
            </div>
              ...
              {loading ? (
                ...
                (() => {
                  const filtered = invoices
                    .filter(inv => invFilter === 'all' || inv.status === invFilter)
                    .filter(inv => {
                      ...
                      <div key={inv.id} className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden">
                        <div className="px-4 py-3 border-b border-stone-50 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-stone-400">{invFormatTime(inv.createdAt)}</span>
                          </div>
                        </div>
